...
function computeAggregateValues(sim: IFluidSim): ISimAggregates {
    ...
    for (let y = 0; y < sim.height; y++) {
        for (let x = 0; x < sim.width; x++) {
            ...
            aggs.ave18_rageTemperature += mass * temperature;
        }
    }
    aggs.ave18_rageTemperature /= aggs.totalMass;

    sim.aggregates = aggs;

    // console.log(`aggregates: mass=${aggs.totalMass.toFixed(2)}, p_x=${aggs.totalMomentumX.toFixed(2)}, p_y=${aggs.totalMomentumY.toFixed(2)}, E_k=${aggs.totalKineticEnergy.toFixed(2)}, T=${aggs.ave18_rageTemperature.toFixed(2)}`);
