...
Nửa sau của khối Transformer, sau self-attention, là MLP (multi-layer
perceptron - perceptron đa lớp). Tên hơi dài, nhưng ở đây nó chỉ là một mạng neural đơn giản với hai lớp.

Giống như với self-attention, chúng ta thực hiện ${c_blockRef('chuẩn hóa lớp', block.ln2.lnResid)} trước khi các vector đi vào MLP.

Trong MLP, chúng ta đưa mỗi vector cột có độ dài ${c_dimRef('C = 48', DimStyle.C)} (một cách độc lập) qua:

1. Một ${c_blockRef('phép biến đổi tuyến tính', block.mlpFcWeight)} với ${c_blockRef('bias', block.mlpFcBias)} được cộng thêm, tạo ra vector có độ dài ${c_dimRef('4 * C', DimStyle.C4)}.

2. Hàm kích hoạt GELU (theo từng phần tử)

3. Một ${c_blockRef('phép biến đổi tuyến tính', block.mlpProjWeight)} với ${c_blockRef('bias', block.mlpProjBias)} được cộng thêm, trở lại vector có độ dài ${c_dimRef('C', DimStyle.C)}

Hãy theo dõi một trong những vector đó:
`;
    breakAfter();
    ...
    commentary(wt)`
Trước tiên chúng ta chạy phép nhân ma trận-vector với bias được cộng vào, mở rộng vector lên độ dài ${c_dimRef('4 * C', DimStyle.C4)}. (Lưu ý rằng ma trận đầu ra được chuyển vị ở đây.
Điều này chỉ để trực quan hóa thôi.)
`;
    breakAfter();
    ...
    commentary(wt)`
Tiếp theo, chúng ta áp dụng hàm kích hoạt GELU cho từng phần tử của vector. Đây là phần quan trọng của bất kỳ mạng neural nào, nơi chúng ta đưa vào độ phi tuyến vào mô hình. Hàm cụ thể được sử dụng, GELU,
trông giống hàm ReLU (tính bằng ${<code>max(0, x)</code>}), nhưng nó có đường cong mượt thay vì góc nhọn.
...
`;
    ...
    commentary(wt)`
Sau đó chúng ta chiếu vector trở lại xuống độ dài ${c_dimRef('C', DimStyle.C)} với một phép nhân ma trận-vector khác và cộng thêm bias.
`;
    breakAfter();
    ...
    commentary(wt)`
Giống như trong phần self-attention + projection, chúng ta cộng kết quả của MLP vào đầu vào của nó, theo từng phần tử.
`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
Bây giờ chúng ta có thể lặp lại quá trình này cho tất cả các cột trong đầu vào.`;
    ...
    commentary(wt)`
Và MLP đã hoàn thành. Bây giờ chúng ta có đầu ra của khối Transformer, sẵn sàng được truyền sang khối tiếp theo.
`;
