...
export function walkthroughIntro(args: IWalkthroughArgs) {
    ...
    let c0 = commentary(wt, null, 0)`Chào mừng bạn đến với hướng dẫn về mô hình ngôn ngữ lớn GPT! Ở đây chúng ta sẽ khám phá mô hình _nano-gpt_, với chỉ 85.000 tham số.

Mục tiêu của nó rất đơn giản: nhận một chuỗi sáu chữ cái: ${embed(ExampleInputOutput)}
và sắp xếp chúng theo thứ tự bảng chữ cái, tức là thành "ABBBCC".`;
    ...
    commentary(wt, t6)`Chúng ta gọi mỗi chữ cái này là một ${tokenStr}, và tập hợp các token khác nhau của mô hình tạo nên _từ vựng_:${embed(TokenVocab)}

    Từ bảng này, mỗi token được gán một số, tức là ${tokenIdxStr}. Và bây giờ chúng ta có thể nhập chuỗi số này vào mô hình:${embed(ExampleTokenValues)}\n`;
    breakAfter();
    ...
    let c5 = commentary(wt)`Trong chế độ xem 3D, mỗi ô xanh lá đại diện cho một số đang được xử lý, và mỗi ô xanh dương là một trọng số. ${embed(GreenBlueCells)}
    Mỗi số trong chuỗi đầu tiên được chuyển thành một vector 48 phần tử (kích thước được chọn cho mô hình cụ thể này). Điều này được gọi là _embedding_ (nhúng).`;
    breakAfter(c5);
    ...
    breakAfter();
    commentary(wt)`Embedding sau đó được truyền qua mô hình, đi qua một loạt các lớp, gọi là transformer, trước khi đến đáy.`;
    breakAfter();
    ...
    commentary(wt)`Vậy đầu ra là gì? Một dự đoán về token tiếp theo trong chuỗi. Vì vậy ở vị trí thứ 6, chúng ta nhận được xác suất token tiếp theo sẽ là
        'A', 'B', hoặc 'C'.`

    commentary(wt)`Trong trường hợp này, mô hình khá chắc chắn đó sẽ là 'A'. Bây giờ, chúng ta có thể đưa dự đoán này trở lại vào đầu mô hình, và lặp lại
    toàn bộ quá trình.`;
        ...
        <table className={s.table}>
            <tbody>
                ...
                <tr className={s.tokIndex} style={{ color: dimStyleColor(DimStyle.TokenIdx).toHexColor() }}>
                    <th>chỉ số</th><td>0</td><td>1</td><td>2</td>
                </tr>
            </tbody>
        </table>
    </div>
...
const GreenBlueCells: React.FC = () => {
    ...
    let blueColor = Vec4.fromHexColor('#A27B5B'); // Creme Brulee
    let greenColor = Vec4.fromHexColor('#DCD7C9'); // Au Lait
