...
Ma trận ${c_blockRef('_input embedding_', state.layout.residual0)} từ phần trước là đầu vào cho khối Transformer đầu tiên của chúng ta.

Bước đầu tiên trong khối Transformer là áp dụng _chuẩn hóa lớp_ (layer normalization) cho ma trận này. Đây là một
phép toán chuẩn hóa các giá trị trong mỗi cột của ma trận một cách riêng biệt.`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
Chuẩn hóa là một bước quan trọng trong việc huấn luyện mạng neural sâu, và nó giúp cải thiện
tính ổn định của mô hình trong quá trình huấn luyện.

Chúng ta có thể xem xét từng cột riêng biệt, vậy hãy tập trung vào cột thứ 4 (${c_dimRef('t = 3', DimStyle.T)}) trước.`;
    ...
    breakAfter();
    commentary(wt)`
Mục tiêu là làm cho giá trị trung bình trong cột bằng 0 và độ lệch chuẩn bằng 1. Để làm điều này,
chúng ta tìm cả hai đại lượng này (${c_blockRef('trung bình (μ)', ln.lnAgg1)} & ${c_blockRef('độ lệch chuẩn (σ)', ln.lnAgg2)}) cho cột và sau đó trừ đi giá trị trung bình rồi chia cho độ lệch chuẩn.`;
    ...
    breakAfter();
    commentary(wt)`
Ký hiệu chúng ta sử dụng ở đây là E[x] cho giá trị trung bình và Var[x] cho phương sai (của cột có độ dài ${c_dimRef('C', DimStyle.C)}). 
Phương sai đơn giản là bình phương của độ lệch chuẩn. Số hạng epsilon (ε = ${<>1&times;10<sup>-5</sup></>}) ở đây để tránh chia cho không.

Chúng ta tính toán và lưu trữ các giá trị này trong lớp tổng hợp vì chúng ta đang áp dụng chúng cho tất cả giá trị trong cột.

Cuối cùng, sau khi có các giá trị đã chuẩn hóa, chúng ta nhân mỗi phần tử trong cột với một
${c_blockRef('trọng số học được (γ)', ln.lnSigma)} và sau đó cộng thêm giá trị ${c_blockRef('bias (β)', ln.lnMu)}, tạo ra ${c_blockRef('các giá trị đã chuẩn hóa', ln.lnResid)} của chúng ta.`;
    ...
    breakAfter();
    commentary(wt)`
Chúng ta chạy phép toán chuẩn hóa này trên mỗi cột của ${c_blockRef('ma trận input embedding', layout.residual0)}, và kết quả là
${c_blockRef('input embedding đã chuẩn hóa', ln.lnResid)}, sẵn sàng được truyền vào lớp Self-Attention.
`;
