...
import { ChapterToCPanel, Commentary, jumpPhase } from './Commentary';
import { WelcomePopup } from './WelcomePopup';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { Resizer } from '../utils/Resizer';
import f from './FutureLayout.module.css';
import { TopStatsBar } from './components/FutureLayoutComponents';
...
export function LayerView() {
    ...
    return (
        <div className={f.futureContainer}>
            <div className={f.futureBgGrid} />
            <div className={`${f.futureOrb} ${f.orbCyan}`} />
            <div className={`${f.futureOrb} ${f.orbMagenta}`} />

            <div className={f.centerCanvasArea}>
                {mainView}
            </div>
            ...
            {canvasRender && (
                <ProgramStateContext.Provider value={canvasRender.progState}>
                    <TopStatsBar />
                    {/* Left HUD Area: Diagram and Menu */}
                    <div style={{ position: 'absolute', left: '20px', top: '80px', bottom: '20px', width: '420px', pointerEvents: 'none', display: 'flex', flexDirection: 'column', gap: '20px', zIndex: 100 }}>
                        <div style={{ pointerEvents: 'all', flex: 1, overflow: 'hidden' }}>
                            <ChapterToCPanel />
                        </div>
                    </div>

                    {/* Right HUD Area: Content */}
                    <div style={{ position: 'absolute', right: '20px', top: '80px', bottom: '20px', width: '380px', pointerEvents: 'none', display: 'flex', flexDirection: 'column', zIndex: 100 }}>
                        <div className={s.sidebar} style={{ pointerEvents: 'all', flex: 1, position: 'relative', width: '100%', display: 'flex', flexDirection: 'column', background: 'var(--panel-bg)', backdropFilter: 'blur(12px)', border: '1px solid var(--border-glow)', borderRadius: '8px', overflow: 'hidden' }}>
                            <Commentary />
                        </div>
                    </div>
