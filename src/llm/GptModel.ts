...
import { IGptModelConfig, ITensorSet, LayerType, ILayerConfig } from "@/src/utils/tensor";
...
export interface IModelShape {
    ...
    isRMSNorm?: boolean;           // Use RMSNorm instead of LayerNorm
    noBias?: boolean;              // Disable biases in linear layers
    isSquareReLU?: boolean;       // Use Square ReLU activation
}
...
export function setModelInputData(renderState: IRenderState, gptModel: IGpuGptModel, rand: Random) {
    ...
    if (gptModel.shape.n_images && gptModel.shape.n_images > 0) {
        // Dummy image data
        // We need a place to store it. I should have added inputImages buffer.
        // For now, let's just log or assume it exists if I add it.
        // But to make it runnable, I must add the buffer to the interface.
    }
}
...
export function runModel(renderState: IRenderState, gptModel: IGpuGptModel, validationData?: ITensorSet) {
    ...
    for (let blockId = 0; blockId < blocks.length; blockId++) {
        ...
        gl.flush();
        if ('gate' in blocks[blockId]) {
            // MoE Block
            let block = blocks[blockId] as IGpuMoEBlockLayer;
            runRenderPhase(gl, block.ln_1.aggPhase);
            runRenderPhase(gl, block.ln_1.applyPhase);

            // Attn
            runRenderPhase(gl, block.attn.qkvPhase);
            runRenderPhase(gl, block.attn.selfAttendPhase);
            runRenderPhase(gl, block.attn.attnMatrixAggPhase);
            runRenderPhase(gl, block.attn.attnMatrixSoftmaxPhase);
            runRenderPhase(gl, block.attn.scaledVectorsPhase);
            runRenderPhase(gl, block.attn.proj.linearPhase);

            gl.flush();
            runRenderPhase(gl, block.attn.add.addPhase);

            runRenderPhase(gl, block.ln_2.aggPhase);
            runRenderPhase(gl, block.ln_2.applyPhase);

            // MoE specific
            runRenderPhase(gl, block.gate.linearPhase);
            runRenderPhase(gl, block.gateSoftmax.aggPhase);
            runRenderPhase(gl, block.gateSoftmax.softmaxPhase);

            // Run experts (all of them for now? or just visualize them?)
            for (let expert of block.experts) {
                runRenderPhase(gl, expert.fcLayer.linearPhase);
                runRenderPhase(gl, expert.geluPhase);
                runRenderPhase(gl, expert.projLayer.linearPhase);
                // We need to handle the add/aggregation properly.
            }
        } else {
            // Standard Block
            let block = blocks[blockId] as IGpuGptBlockLayer; // Cast to specific type to avoid lint errors
            runRenderPhase(gl, block.ln_1.aggPhase);
            runRenderPhase(gl, block.ln_1.applyPhase);
            runRenderPhase(gl, block.attn.qkvPhase);
            runRenderPhase(gl, block.attn.selfAttendPhase);
            ...
            runRenderPhase(gl, block.ln_2.aggPhase);
            runRenderPhase(gl, block.ln_2.applyPhase);
            runRenderPhase(gl, block.mlp.fcLayer.linearPhase);
            runRenderPhase(gl, block.mlp.geluPhase);
            runRenderPhase(gl, block.mlp.projLayer.linearPhase);
            runRenderPhase(gl, block.mlp.addLayer.addPhase);

            gl.flush();
            validate(`block${blockId}`, block.mlp.addLayer.addPhase);
        }
...
export interface IGptModelLink {
    ...
    inputImages?: IBufferTex;
    vocabEmbed: IEmbedLayerLink;
    posEmbed: IEmbedLayerLink;
    add: IAddLayerLink;
    blocks: (IBlockLayerLink | IGpuMoEBlockLayer)[],
...
export interface IGpuGptModel extends IGptModelLink {
    ...
    blocks: (IGpuGptBlockLayer | IGpuMoEBlockLayer)[];
...
export function createGptModel(shaderManager: IShaderManager, model: ITensorSet, B: number): IGpuGptModel {
    ...
    let nImages = config.n_images || 0;
    let A = C / nHeads; // n elements in each Q, K, V vector, i.e. what we project down to

    let shape: IModelShape = { B, C, nHeads, T, A, nBlocks, vocabSize, n_images: nImages };
    ...
    let inputImages = nImages > 0 ? createBufferTex(gl, 1, nImages * 32 * 32, 1) : undefined;
    ...
    let blocks: (IGpuGptBlockLayer | IGpuMoEBlockLayer)[] = [];
    let x = add.output;

    if (config.layers) {
        for (let i = 0; i < config.layers.length; i++) {
            let layerConfig = config.layers[i];
            if (layerConfig.type === LayerType.MoE) {
                let block = createMoEBlockLayer(layerBuilder, prefix + '.h.' + i, x, layerConfig as any); // Cast needed until strict types
                blocks.push(block);
                x = block.output;
            } else {
...
function createMLP(layerBuilder: ILayerBuilder, prefix: string, input: IBufferTex, residual: IBufferTex): IGpuMlpLayer {
    ...
    // operating memory
    let mlpGelu = createBufferTex(gl, C * 4, B * T, 1, [B, T, C * 4]); // (B, T) (4C)
...
export interface IGpuMoELayer extends IBlockLayerLink {
    gate: IGpuLinearLayer;
    gateSoftmax: IGpuSoftmaxLayer;
    experts: IGpuMlpLayer[];
    // We might need a mix/reduce phase if we don't just sum them up
    // For now, let's assume we just sum the active experts
    // output: IBufferTex; // Inherited from IBlockLayerLink? No, IBlockLayerLink has output
    // But IBlockLayerLink demands ln_1, ln_2, attn, mlp.
    // MoE replaces MLP in a block, or is it a layer itself?
    // Usually MoE replaces the FeedForward part of a transformer block.
    // So a Transformer block has (SelfAttention + MoE) instead of (SelfAttention + MLP).
    // Let's assume for this viz, a "Layer" is the whole residual block.
    // So we need a `IGpuMoEBlockLayer` that looks like `IGpuGptBlockLayer` but has `experts` instead of `mlp`.
}

export interface IGpuMoEBlockLayer extends IBlockLayerLink {
    ln_1: IGpuLayerNormLayer;
    ln_2: IGpuLayerNormLayer;
    attn: IGpuAttnLayer;
    ...
    // We need to satisfy IBlockLayerLink which has `mlp`.
    // We can make `mlp` optional in IBlockLayerLink or use a union type for `blocks` in the model.
    // Let's modify IBlockLayerLink to be more flexible or just ignore it for now and cast.
    mlp: IGpuMlpLayer; // Placeholder to satisfy IBlockLayerLink
}

function createMoEBlockLayer(layerBuilder: ILayerBuilder, prefix: string, input: IBufferTex, config: { n_experts: number, experts_per_token: number }): IGpuMoEBlockLayer {
    let ln_1 = createLayerNorm(layerBuilder, prefix + '.ln_1', input);
    let attn = createAttnLayer(layerBuilder, prefix + '.attn', ln_1.output, input);
    let ln_2 = createLayerNorm(layerBuilder, prefix + '.ln_2', attn.output);
    ...
    // Gate
    // Experts
    // We need to implement the MoE specific parts here.
    // For now, let's create a placeholder MoE structure.

    // Config: n_experts
    let nExperts = config.n_experts;

    // Gate: Linear layer (C -> n_experts)
    let gate = createLinearLayer(layerBuilder, prefix + '.gate', layerBuilder.shape.C, nExperts, ln_2.output);
    let gateSoftmax = createSoftmaxLayer(layerBuilder, gate.output);

    let experts = [];
    for (let i = 0; i < nExperts; i++) {
        // Each expert is an MLP
        // We'll reuse createMLP but we need to adjust the prefix and maybe inputs
        // Experts are usually executed on the same input (ln_2.output)
        // AND then weighted by the gate.
        // Visualization-wise, we might want to show them all? Or just the active ones?
        // Let's create them all for now.
        let expert = createMLP(layerBuilder, prefix + '.experts.' + i, ln_2.output, attn.output /* This residual connection is wrong for MoE inner, the residual depends on the whole block */);
        experts.push(expert);
    }

    // Note: The residual connection in MoE is usually:
    // x = x + attn(ln1(x))
    // x = x + moe(ln2(x))
    // The `createMLP` function adds the residual internally: `createAddLayer(..., residual)`.
    // So if we pass `attn.output` as residual to EACH expert, they will all add to it?
    // That's not right for visualization.
    // We probably need a custom aggregation of experts.
    ...
    return {
        ...
        gateSoftmax,
        experts,
        output: experts[0].output, // Placeholder!
        mlp: experts[0], // Placeholder to satisfy IBlockLayerLink
    };
}
