...
export function mkSubscript(text: string, opts: IFontOpts): ITextBlockArgs {
    return {
        text,
        opts: { ...opts, size: opts.size * 0.7 },
        yOffset: opts.size * 0.25,
    };
}

export function mkSuperscript(text: string, opts: IFontOpts): ITextBlockArgs {
    return {
        text,
        opts: { ...opts, size: opts.size * 0.7 },
        yOffset: -opts.size * 0.25,
...
export function layoutBlock(blk: ITextBlock) {
    switch (blk.type) {
        case TextBlockType.Line: {
            ...
            for (let sub of blk.subs!) {
                sub.offset = new Vec3(x, midY - sub.size.y / 2 + sub.yOffset).round_();
                layoutBlock(sub);
                x += sub.size.x;
            }
            break;
