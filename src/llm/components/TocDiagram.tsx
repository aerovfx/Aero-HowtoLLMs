...
export const TocDiagram: React.FC<{
    ...
    let setActive = useCallback((ev: React.MouseEvent, id: Phase, active: boolean) => {
        console.log('[TocDiagram] setActive called with phase:', id, 'Phase.MicroGPT_Intro:', Phase.MicroGPT_Intro);
        if (progState) {
            // Auto-switch to MicroGPT model when jumping to MicroGPT phases
            if (id === Phase.MicroGPT_Intro) {
                console.log('[TocDiagram] Detected MicroGPT phase, attempting auto-switch');
                console.log('[TocDiagram] Available examples:', progState.examples.map(ex => ex.name));
                let microGptIdx = progState.examples.findIndex(ex => ex.name === 'MICRO-GPT');
                console.log('[TocDiagram] MicroGPT index:', microGptIdx, 'Current example ID:', progState.currExampleId);
                if (microGptIdx !== -1 && progState.currExampleId !== microGptIdx) {
                    console.log('[TocDiagram] Switching to MicroGPT model');
                    progState.currExampleId = microGptIdx;
                    progState.markDirty(); // Trigger re-render after model switch
                } else {
                    console.log('[TocDiagram] Skip switch - already on MicroGPT or not found');
                }
            }
    ...
    }, [progState, onEnterPhase]);

    activeId = activePhase ?? activeId;

    let colors = {
        ln: '#00f0ff', // Cyan
        multihead: '#ffaa00', // Amber
        feedForward: '#00f0ff', // Cyan
        tokEmbed: '#ff0080', // Magenta
        linear: '#00f0ff', // Cyan
        softmax: '#ff0080', // Magenta
        line: '#ffffff',
        focus: '#ff0080', // Magenta
    };
    ...
    // Get current model shape to build appropriate structure
    let currShape = progState?.examples[progState.currExampleId]?.shape ?? progState?.mainExample.shape;
    let isMicroGPT = currShape?.isRMSNorm || currShape?.nBlocks === 1;

    let structure: IEl = {
        type: ElType.Block, height: 0, color: 'rgba(255, 255, 255, 0.05)', special: 'llm', padX: 2.0, padY: 1.0, id: 'llm', items: [
            { type: ElType.Cell, label: 'Nhúng Token', height: 1, width: 4, color: colors.tokEmbed, id: 'tokEmbed' },
            { type: ElType.PosEmbed, height: 1.8, id: 'posEmbed' },
            {
                type: ElType.Block, height: 0, padX: 3, padY: 0, color: 'rgba(255, 255, 255, 0.03)', special: 'transformer', id: 'transformer', items: [
                    { type: ElType.Gap, height: 1.2, gapType: 'exit', arrow: true },
                    { type: ElType.Cell, height: 1, label: isMicroGPT ? 'RMS Norm' : 'Chuẩn hóa lớp', width: widthStd, color: colors.ln, id: 'ln1' },
                    { type: ElType.Gap, height: exitGap, gapType: 'multihead', arrow: true },
                    { type: ElType.Cell, height: 2, label: isMicroGPT ? ['Tự chú ý', '(đơn giản)'] : ['Đa đầu, tự chú ý', 'có điều kiện'], width: widthMultihead, color: colors.multihead, id: 'selfAttend' },
                    { type: ElType.Gap, height: plusGap, gapType: 'add' },
                    { type: ElType.Gap, height: exitGap, gapType: 'exit', arrow: true },
                    { type: ElType.Cell, height: 1, label: isMicroGPT ? 'RMS Norm' : 'Chuẩn hóa lớp', width: widthStd, color: colors.ln, id: 'ln2' },
                    { type: ElType.Gap, height: smallGap, arrow: true },
                    { type: ElType.Cell, height: 2, label: isMicroGPT ? ['MLP', '(Square ReLU)'] : ['Mạng lan truyền', 'tiến (FFN)'], width: widthFeedForward, color: colors.feedForward, id: 'feedForward' },
                    { type: ElType.Gap, height: plusGap, gapType: 'add' },
                ]
            },
            { type: ElType.Gap, height: smallGap, arrow: true },
            { type: ElType.Cell, height: 1, label: isMicroGPT ? 'RMS Norm' : 'Chuẩn hóa lớp', width: widthStd, color: colors.ln, id: 'lnf' },
            { type: ElType.Gap, height: smallGap, arrow: true },
            { type: ElType.Cell, label: isMicroGPT ? 'Tuyến tính (no bias)' : 'Tuyến tính', height: 1, width: widthStd, color: colors.linear, id: 'linear' },
            { type: ElType.Gap, height: smallGap, arrow: true },
    ...
    entryGroups.push({ groupName: 'Giới thiệu', entries: [] });
    makeEntry(Phase.Intro_Intro, 'Mở đầu', []);
    makeEntry(Phase.Intro_Prelim, 'Kiến thức nền', []);

    // Always show MicroGPT section
    entryGroups.push({ groupName: 'MicroGPT', entries: [] });
    makeEntry(Phase.MicroGPT_Intro, 'Giới thiệu MicroGPT', []);

    // Component section with model-specific labels
    entryGroups.push({ groupName: 'Thành phần', entries: [] });
    makeEntry(Phase.Input_Detail_Embedding, 'Nhúng (Embedding)', ['tokEmbed', 'posEmbed'], true);

    if (isMicroGPT) {
        makeEntry(Phase.Input_Detail_LayerNorm, 'RMS Norm', ['ln1', 'ln2', 'lnf']);
        makeEntry(Phase.Input_Detail_SelfAttention, 'Tự chú ý (Self Attention)', ['selfAttend']);
        makeEntry(Phase.Input_Detail_Mlp, 'MLP (Square ReLU)', ['feedForward']);
    } else {
        makeEntry(Phase.Input_Detail_LayerNorm, 'Chuẩn hóa lớp (Layer Norm)', ['ln1', 'ln2', 'lnf']);
        makeEntry(Phase.Input_Detail_SelfAttention, 'Tự chú ý (Self Attention)', ['selfAttend']);
        makeEntry(Phase.Input_Detail_Projection, 'Chiếu (Projection)', ['selfAttend']);
        makeEntry(Phase.Input_Detail_Mlp, 'Mạng nơ-ron (MLP)', ['feedForward']);
    }

    makeEntry(Phase.Input_Detail_Transformer, isMicroGPT ? 'Khối Transformer' : 'Cấu trúc Transformer', ['transformer']);
    makeEntry(Phase.Input_Detail_Softmax, 'Hàm Softmax', ['softmaxOut']);
    makeEntry(Phase.Input_Detail_Output, 'Kết quả đầu ra', ['lnf', 'linear', 'softmaxOut'], true);
        ...
        switch (el.type) {
            ...
            case ElType.Block:
                ...
                content = <>
                    ...
                    {renderResiduals()}
                    {el.special === 'transformer' && <text fontSize={13} textAnchor='start' x={4} y={14} fill={'var(--cyan)'}>Transformer i</text>}
                    {el.special === 'llm' && <text fontSize={13} textAnchor='start' x={4} y={14} fill={'var(--amber)'}>Mô hình LLM</text>}
                </>;
                opacityDimmed = 1.0;
                break;
            case ElType.Gap:
    ...
    function renderExampleText() {
        ...
        let node = <g transform={`translate(${x} ${y})`}>
            {textSegs.map((seg, i) => {
                return <React.F18_ragment key={i}>
