...
import { ITextBlock, sizeBlock, layoutBlock, drawBlock, mkTextBlock, TextBlockType, drawCells, ITextBlockArgs, mkSubscript, mkSuperscript } from "./TextLayout";
...
export function drawOLMatrixMul(args: IDataFlowArgs) {
    ...
    function hybridCell(dep: IBlkCellDep, label: string) {
        let isRow = dep.srcIdxMtx.g(0, 3) === 1.0;
        let isWeight = dep.src.t === 'w';
            ...
            subs: [
                {
                    cellX: isRow ? 3 : 1,
                    cellY: isRow ? 1 : 3,
                    color: isWeight ? weightSrcColor : workingSrcColor,
                },
                {
                    text: label,
                    color: isWeight ? weightSrcColor : workingSrcColor,
                    opts: { ...fontOpts, size: fontOpts.size * 1.0 }
                },
            ],
        };
    }
    ...
    let textBlock = mkTextBlock({
        ...
        subs: [
            hasAdd ? { text: 'b + ' } : null,
            { text: 'dot(' },
            hybridCell(dotA, dotA.src.t === 'w' ? 'W' : 'x'),
            { text: ', ' },
            hybridCell(dotB, dotB.src.t === 'w' ? 'W' : 'x'),
            { text: ')' },
        ].filter(isNotNil),
    });
...
function drawLayerNormMuAgg(args: IDataFlowArgs) {
    ...
    let textBlock = mkTextBlock({
        opts: fontOpts,
        subs: [
            { text: '\u03bc = E(', color: workingSrcColor },
            { text: 'x', color: workingSrcColor },
            ...
            subs: [
                {
                    subs: [
                        { text: 'x', color: workingSrcColor },
                        { text: ' \u2212 ' },
                        { text: '\u03bc', color: workingSrcColor },
                    ],
                },
                {
                    subs: [
                        { text: '\u03c3', color: workingSrcColor },
                    ],
                }]
        },
        { text: ' \u00b7 ' },
...
function drawResidualAdd(args: IDataFlowArgs) {
    ...
    let textBlock = mkTextBlock({
        ...
        subs: [
            ...
            { text: ' + ' },
            { text: 'y', opts: { ...fontOpts, color: workingSrcColor } },
        ],
    });
...
export function drawAttention(args: IDataFlowArgs) {
    ...
    function hybridCell(dep: IBlkCellDep, qkvLabel: string) {
        let isRow = dep.srcIdxMtx.g(0, 3) === 1.0;
        let isWeight = dep.src.t === 'w';
        ...
        return {
            ...
            subs: [
                ...
                },
                {
                    text: qkvLabel,
                    color: isWeight ? weightSrcColor : workingSrcColor,
                    opts: { ...fontOpts, size: fontOpts.size * 1.1 }
                },
            ],
        };
    }
...
export function drawGeluActivation(args: IDataFlowArgs) {
    ...
    let fontOpts: IFontOpts = { color: opColor, mtx, size: 14 };
    ...
    drawMaths(args, center.add(new Vec3(0, 15, 0)), textBlk);
