...
export function moveSelectedComponents(schematic: IEditSchematic, selected: IElRef[], delta: Vec3): IEditSchematic {
    ...
    return assignImm(schematic, {
        ...
        wires: schematic.wires.map(wire => {
            ...
            if (nodeIdsToMove) {
                wire = d18_ragNodes(wire, nodeIdsToMove);
            }
            return wire;
        }),
    });
...
export function d18_ragNodes(wire: IWireGraph, nodesToMove: Map<number, Vec3>) {
    // kinda complicated, but assume we're d18_ragging a component (or 3) with wires attached to their nodes

    // we do something similar to segment d18_ragging, where we try to extend down co-linear segments
...
export function d18_ragSegment(wire: IWireGraph, node0Idx: number, node1Idx: number, delta: Vec3) {
