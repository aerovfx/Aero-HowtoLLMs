...
2. **Hạn chế của Python Thuần (Base Python):** Python cơ bản không cung cấp sẵn hàm `mean()` hay `average()`. Để thực hiện các phép toán thống kê này, lập trình viên phải tự xây dựng thuật toán hoặc sử dụng các thư viện chuyên dụng như NumPy.
