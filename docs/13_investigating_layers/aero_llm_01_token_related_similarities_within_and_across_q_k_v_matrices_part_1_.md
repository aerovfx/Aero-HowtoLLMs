...
$$
\text{Cosine Similarity}(\mathbf{x}, \mathbf{y}) = \frac{\mathbf{x} \cdot \mathbf{y}}{\|\mathbf{x}\| \|\mathbf{y}\|} = \frac{$\sum$_{i=1}^{n} x_i y_i}{\sqrt{$\sum$_{i=1}^{n} x_i^2}\sqrt{$\sum$_{i=1}^{n} y_i^2}}
$$
