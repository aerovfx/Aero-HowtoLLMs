...
$$
Output: [batch_size, seq_len, vocab]    = [5, 8, 100000] ### 5.2 So Sánh `nn.Embedding` vs `nn.Linear` | Khía cạnh | nn.Embedding | nn.Linear | |-----------|--------------|-----------| | Mục đích | Tra cứu vector từ bảng | Phép biến đổi tuyến tính | | Input | Token indices (integers) | Dense vectors (floats) | | Cơ chế | Lookup operation | Matrix multiplication | | Gradient | Chỉ update vectors được dùng | Update toàn bộ ma trận | | Hiệu quả | Tối ưu cho sparse inputs | Tối ưu cho dense inputs | **Điểm chung:** Về bản chất, cả hai đều thực hiện phép nhân ma trận, nhưng với interface và tối ưu hóa khác nhau. ### 5.3 Hàm Kích Hoạt GELU vs ReLU **GELU (Gaussian Error Linear Unit):** \text{GELU}(x) = x \cdot \Phi(x) Trong đó Φx là hàm phân phối chuẩn tích lũy. **Ưu điểm của GELU trong LLMs:** - Mượt hơn ReLU (khả vi tại mọi điểm) - Cho phép giá trị âm có trọng số - Hiệu suất tốt hơn trong các mô hình lớn - Được sử dụng trong GPT-2, BERT, và hầu hết LLMs hiện đại **So sánh:**
$$
