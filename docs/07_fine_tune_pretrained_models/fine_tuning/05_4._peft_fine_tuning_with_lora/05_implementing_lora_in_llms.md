...
$$
def call(self, inputs): # Lấy output từ lớp gốc
$$
...
$$
return original_output + lora_output ### 2.5 Thay Thế Các Lớp Dense ```python # Thay thế các lớp dense trong model
$$
...
$$
for layer in model.layers: if isinstance(layer, tf.keras.layers.Dense): # Thay thế bằng LoraDenseLayer
$$
...
$$
return model ## 3. Ví Dụ Hoàn Chỉnh ### 3.1 Huấn Luyện với LoRA ```python # Tải dữ liệu WMT16 from datasets import load_dataset
$$
...
$$
# Tiền xử lý def preprocess(examples):
$$
...
$$
# Compile và huấn luyện model.compile(
$$
